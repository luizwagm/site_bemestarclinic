...
async function conferirSqlite(sq) {
  ...
  checar(
    "Prontuários repetidos para o mesmo paciente + especialidade",
    `SELECT paciente_id, especialidade, COUNT(*) n, GROUP_CONCAT(id) ids
       FROM prontuario GROUP BY paciente_id, especialidade HAVING COUNT(*) > 1`,
    (r) => `paciente ${r.paciente_id} · "${r.especialidade}" · ${r.n} pastas (ids ${r.ids})`
  );
  checar(
    "Números de prontuário repetidos",
    `SELECT numero, COUNT(*) n, GROUP_CONCAT(id) ids FROM prontuario
       WHERE numero IS NOT NULL GROUP BY numero HAVING COUNT(*) > 1`,
    (r) => `${r.numero} · ${r.n} vezes (ids ${r.ids})`
  );
  checar(
    "Códigos de paciente repetidos",
    `SELECT codigo, COUNT(*) n, GROUP_CONCAT(id) ids FROM pacientes
       WHERE codigo IS NOT NULL AND codigo <> '' GROUP BY codigo HAVING COUNT(*) > 1`,
    (r) => `${r.codigo} · ${r.n} vezes (ids ${r.ids})`
  );
  checar(
  ...
  checar("Prontuários sem paciente ou sem especialidade",
    `SELECT id FROM prontuario WHERE paciente_id IS NULL OR especialidade IS NULL OR especialidade=''`,
    (r) => `prontuário id ${r.id}`);
  checar("Anamneses sem paciente ou sem tipo",
    `SELECT id FROM anamneses WHERE paciente_id IS NULL OR tipo IS NULL OR tipo=''`,
    (r) => `anamnese id ${r.id}`);
  checar("Pacientes sem nome",
...
/* ==========================================================================
   ...
   paciente é a clínica, não o script.
   ========================================================================== */
...
async function conferirConteudo(sq) {
  const erros = [];
  const amostras = [
    ["pacientes", "id, nome, cpf, codigo, ativo"],
    ["prontuario", "id, numero, paciente_id, especialidade, status"],
    ["prontuario_registros", "id, prontuario_id, tipo, texto"],
    ["anamneses", "id, paciente_id, tipo, status, dados"],
    ["atendimentos", "id, paciente_id, data, hora, status"],
  ];
