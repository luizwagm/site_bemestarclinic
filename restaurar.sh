...
PGDATABASE_NOME="${PGDATABASE:-bemestar_gestao}"
...
if [ "$BANCO" = "gestao" ]; then
  ...
  if PGPASSWORD="${PGPASSWORD:-}" psql -v ON_ERROR_STOP=1 \
       -h "${PGHOST:-127.0.0.1}" -p "${PGPORT:-5432}" -U "${PGUSER:-bemestar}" \
    ...
    amarelo  "       psql -U ${PGUSER:-bemestar} -d $PGDATABASE_NOME -f $SEGURANCA"
    exit 1
  fi
else
