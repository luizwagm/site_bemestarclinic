...
function config() {
  ...
  return {
    ...
    user: process.env.PGUSER || "bemestar",
    password: process.env.PGPASSWORD || "",
    database: process.env.PGDATABASE || "bemestar_gestao",
  };
}
...
function obterPool() {
  if (!pool) {
    ...
    pool = new Pool({
      ...
      application_name: "bemestar-restrito",
    });
    /* Um erro num cliente OCIOSO do pool (o Postgres reiniciou, a rede caiu)
       ...
       inteiro — a clínica sairia do ar por causa de uma conexão parada. */
...
/* ==========================================================================
   ...
   O systemd entrega /etc/bemestar.env ao SERVIÇO. Mas o deploy.sh, o
...
function carregarAmbiente(raiz) {
  ...
  const tentados = [
    process.env.BEMESTAR_ENV,                    // caminho explícito, se alguém quiser
    "/etc/bemestar.env",                          // produção (o mesmo que o systemd lê)
