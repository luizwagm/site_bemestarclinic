/* ==========================================================================
   ...
   chave em DADOS_CHAVE é o que separa a clínica de um banco ilegível. Guarde-a
...
async function cifrarTudo() {
  ...
  for (const [tabela, campos] of Object.entries(CAMPOS_PROTEGIDOS)) {
    ...
    /* Uma transação por TABELA, e não uma só para tudo: se algo der errado no
       ...
       Uma transação única sobre a clínica inteira seria mais elegante e muito
       pior de recuperar. */
    await Q.tx(async () => {
...
async function principal() {
  ...
  if (!chaveConfigurada()) {
    ...
    console.error("    E grave como DADOS_CHAVE em /etc/bemestar.env (ou no .env, em desenvolvimento).\n");
    process.exit(1);
  }
